...
export default function MainPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      clearTimeout(widgetTimer); // ✅ now this works properly
    };
  }, []);
    ...
    <div className="relative w-full max-w-6xl h-[80vh] sm:h-[83vh] mx-auto bg-gradient-to-br from-[#0f0c29] via-[#302b63] to-[#24243e] text-white rounded-2xl overflow-hidden shadow-2xl border border-white/10">
      ...
      {showCompletion ? (
        ...
        <div className="relative z-20 h-screen flex flex-col justify-center items-center text-center px-4 animate__animated animate__fadeInUp">
          ...
          <button className='inline-block mt-4 px-8 py-3 bg-white text-black font-semibold rounded-full transition duration-300 shadow-lg hover:cursor-pointer hover:bg-violet-500 hover:text-white hover:shadow-zinc-950 animate__animated animate__pulse animate__infinite' onClick={() => router.push('/levels/level2')}>Next Level</button>
        </div>
      ) : (
        ...
        <div className="h-full overflow-y-scroll scrollbar-hidden px-4 py-6 sm:px-6 sm:py-8 space-y-10 z-10 relative">
          ...
          <div className="z-20 relative text-center space-y-4">
            ...
            <p className="text-gray-300 max-w-2xl mx-auto text-sm sm:text-base leading-relaxed">
              Elevate your spoken English with real-time AI assessment. Speak naturally, and get instant, intelligent feedback on fluency, grammar, vocabulary, and clarity — all in just 3 minutes.
            </p>
          </div>
      ...
      {/* Welcome Popup */}
      {showPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-900">
